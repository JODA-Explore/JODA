...
class DirectoryExportThread
    : public IWorkerThread<JsonContainerQueue, NullQueue, std::string> {
 public:
  using IWorkerThread::IWorkerThread;
  ~DirectoryExportThread() override = default;

  static const size_t recommendedThreads() { return config::parsingThreads; }
