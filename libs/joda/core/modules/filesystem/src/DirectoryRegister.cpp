...
bool joda::filesystem::DirectoryRegister::registerDirectory(const fs::path &dir, bool checkDuplicate = true) {
...
bool joda::filesystem::DirectoryRegister::createDir(const fs::path &dir, bool checkExisting) {
  try {
    if(checkExisting){
      if(fs::exists(dir)){
...
bool joda::filesystem::DirectoryRegister::registerDirectory(const std::string &dir, bool checkDuplicate) {
...
bool joda::filesystem::DirectoryRegister::removeDirectory(const fs::path &dir) {
  std::lock_guard<std::mutex> guard(mut);
  if(dirs.find(dir) == dirs.end()){
    LOG(WARNING)<< "Directory '"<<dir.c_str()<< "' not registered but requested to remove.";
    return true;
  }
  if(removeDir(dir)){
    dirs.erase(dir);
...
bool joda::filesystem::DirectoryRegister::removeDir(const fs::path &dir) {
  try{
    ...
    LOG(INFO) <<  "Removed directory '"<<dir.c_str()<< "'. " <<count <<" files and directories deleted.";
    return true;
  }catch(const fs::filesystem_error& e){
    LOG(ERROR)<<e.what();
    return false;
