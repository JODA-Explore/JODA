...
std::vector<std::string> joda::filesystem::DirectoryFileScanner::getFilesInDir(const std::string &directory, const std::string &ending) {
#ifdef WINDOWS
  using namespace std;
  ...
  while ((ent = readdir(dir)) != NULL) {
    ...
    const string full_file_name = directory + 
...
bool joda::filesystem::DirectoryFileScanner::hasEnding(const std::string &file, const std::string &ending) {
...
size_t joda::filesystem::DirectoryFileScanner::getDirectorySize(const std::string &dir, bool recursive = false, const std::string &extension = "") const {
  namespace fs = std::experimental::filesystem;
  auto p = fs::path(dir.c_str());
  if(!fs::is_directory(p))  return 0;
  ...
  if(recursive){
    ...
    {
      if(fs::is_regular_file(*it) && (extension.empty() || it->path().extension().string() == "."+extension))
        ret+=fs::file_size(*it);
    }
  }else{
