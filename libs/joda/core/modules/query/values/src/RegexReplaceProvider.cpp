...
std::unique_ptr<joda::query::IValueProvider> joda::query::RegexReplaceProvider::duplicate() const {
  return std::make_unique<joda::query::RegexReplaceProvider>(duplicateParameters());
}


bool joda::query::RegexReplaceProvider::isConst() const {
  return false;
}


const RJValue *joda::query::RegexReplaceProvider::getValue(const RapidJsonDocument &json,
                                                           RJMemoryPoolAlloc &alloc) const {
  DCHECK(!isAtom())<< "Did not check for atom before calling";
  return nullptr;
}

RJValue joda::query::RegexReplaceProvider::getAtomValue(const RapidJsonDocument &json,
                                                        RJMemoryPoolAlloc &alloc) const {
  DCHECK(isAtom()) << "Did not check for atom before calling";
  std::string toCheck;
  std::string replace;
  RJValue val;
  if(getParamString(toCheck, params[0], json) && getParamString(replace, params[2], json) ){
...
joda::query::RegexReplaceProvider::RegexReplaceProvider(std::vector<std::unique_ptr<joda::query::IValueProvider>> &&parameters) : joda::query::IValueProvider(
    std::move(parameters)) {
  checkParamSize(3);
...
joda::query::IValueType joda::query::RegexReplaceProvider::getReturnType() const {
  return IV_String;
}
