...
std::unique_ptr<joda::query::IValueProvider> joda::query::RegexExtractProvider::duplicate() const {
  return std::make_unique<joda::query::RegexExtractProvider>(duplicateParameters());
}


bool joda::query::RegexExtractProvider::isConst() const {
  return false;
}

RJValue joda::query::RegexExtractProvider::getAtomValue(const RapidJsonDocument &json,
...
bool joda::query::RegexExtractProvider::isAtom() const {
  return true;
}

const RJValue *joda::query::RegexExtractProvider::getValue(const RapidJsonDocument &json,
                                                           RJMemoryPoolAlloc &alloc) const {
  DCHECK(!isAtom())<< "Did not check for atom before calling";
  return nullptr;
}

joda::query::RegexExtractProvider::RegexExtractProvider(std::vector<std::unique_ptr<joda::query::IValueProvider>> &&parameters) : joda::query::IValueProvider(
    std::move(parameters)) {
  checkParamSize(2);
...
joda::query::IValueType joda::query::RegexExtractProvider::getReturnType() const {
  return IV_Array;
}
