...
joda::query::TypeProvider::duplicate() const {
  return std::make_unique<TypeProvider>(duplicateParameters());
}

bool joda::query::TypeProvider::isConst() const { return false; }

RJValue joda::query::TypeProvider::getAtomValue(
...
const RJValue* joda::query::TypeProvider::getValue(
    const RapidJsonDocument& /*json*/, RJMemoryPoolAlloc& /*alloc*/) const {
  assert(!isAtom() && "Did not check for atom before calling");
  return nullptr;
}

std::string joda::query::TypeProvider::getName() const { return "TYPE"; }

joda::query::IValueType joda::query::TypeProvider::getReturnType() const {
  return IV_String;
}

joda::query::TypeProvider::TypeProvider(
