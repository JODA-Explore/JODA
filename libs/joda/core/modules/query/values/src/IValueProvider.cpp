...
void joda::query::IValueProvider::replaceConstSubexpressions(std::unique_ptr<IValueProvider> &val) {
  ...
  if (!val->params.empty() && val->isConst()) {
    ...
    if (lhs->IsNull()) val = std::make_unique<NullProvider>();
    else if (lhs->IsTrue()) val = std::make_unique<BoolProvider>(true);
    else if (lhs->IsFalse()) val = std::make_unique<BoolProvider>(false);
    else if (lhs->IsString()) val = std::make_unique<StringProvider>(lhs->GetString());
    else if (lhs->IsUint64()) val = std::make_unique<UInt64Provider>(lhs->GetUint64());
    else if (lhs->IsInt64()) val = std::make_unique<Int64Provider>(lhs->GetInt64());
    else if (lhs->IsNumber()) val = std::make_unique<DoubleProvider>(lhs->GetDouble());
    else if (lhs->IsObject()) DLOG(WARNING) << "IsObject, but no Object Provider to replace";
    else if (lhs->IsArray()) DLOG(WARNING) << "IsArray, but no Array Provider to replace";
...
bool joda::query::IValueProvider::constBoolCheck(std::unique_ptr<IValueProvider> &val) {
  if (val->isBool() && val->isConst()
      && !((val->toString() == JODA_TRUE_STRING) || (val->toString() == JODA_FALSE_STRING))) {
    RJMemoryPoolAlloc alloc;
    auto bVal = val->getAtomValue(RapidJsonDocument(), alloc);
    if (bVal.IsBool()) {
      LOG(INFO) << "\"" + val->toString() << "\" evaluates to "
                << (bVal.GetBool() ? JODA_TRUE_STRING : JODA_FALSE_STRING) << " and was replaced";
      val = std::move(std::make_unique<BoolProvider>(bVal.GetBool()));
      return true;
    }
  }
  return false;
...
joda::query::IValueProvider::IValueProvider(std::vector<std::unique_ptr<IValueProvider>> &&parameters) : params(std::move(parameters)) {
}

joda::query::IValueProvider::IValueProvider() : IValueProvider(std::vector<std::unique_ptr<IValueProvider>>()){
...
void joda::query::IValueProvider::checkParamType(unsigned int i, IValueType expected) {
