...
joda::query::FileNameProvider::duplicate() const {
  return std::make_unique<joda::query::FileNameProvider>(duplicateParameters());
}

bool joda::query::FileNameProvider::isConst() const { return false; }

RJValue joda::query::FileNameProvider::getAtomValue(
...
const RJValue* joda::query::FileNameProvider::getValue(
    const RapidJsonDocument& /*json*/, RJMemoryPoolAlloc& /*alloc*/) const {
  DCHECK(!isAtom() && "Did not check for atom before calling");
  return nullptr;
}
...
joda::query::FileNameProvider::FileNameProvider(
