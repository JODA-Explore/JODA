...
joda::query::ListAttributesProvider::duplicate() const {
  return std::make_unique<joda::query::ListAttributesProvider>(
      duplicateParameters());
}

bool joda::query::ListAttributesProvider::isConst() const { return false; }

RJValue joda::query::ListAttributesProvider::getAtomValue(
  ...
  if (params[0]->isObject() || params[0]->isAny()) {
    ...
    if (target != nullptr && target->IsObject()) {
      ...
      for (auto& m : target->GetObject()) {
        ...
        if (recursive && m.value.IsObject()) {
          recursiveAdd(m.name.GetString(), val, m.value, alloc);
        }
      }
    }
  }
...
const RJValue* joda::query::ListAttributesProvider::getValue(
    const RapidJsonDocument& /*json*/, RJMemoryPoolAlloc& /*alloc*/) const {
  return nullptr;
}

bool joda::query::ListAttributesProvider::isAtom() const { return true; }

joda::query::ListAttributesProvider::ListAttributesProvider(
    std::vector<std::unique_ptr<IValueProvider>>&& parameters)
    : IValueProvider(std::move(parameters)) {
  if (params.empty()) {
    throw joda::query::WrongParameterCountException(params.size(), 1,
                                                    getName());
  }
  checkParamType(0, IV_Object);
  if (params.size() > 2) {
    throw joda::query::WrongParameterCountException(params.size(), 2,
                                                    getName());
  }
  if (params.size() == 2) {
    ...
    if (!this->params[1]->isConst()) {
      throw joda::query::WrongParameterException(
          "Parameter 1 has to be const bool");
    }
...
joda::query::IValueType joda::query::ListAttributesProvider::getReturnType()
    const {
  return IV_Array;
}

void joda::query::ListAttributesProvider::recursiveAdd(
    const std::string& prefix, RJValue& arr, const RJValue& obj,
  ...
  if (obj.IsObject()) {
    for (auto& m : obj.GetObject()) {
      ...
      if (m.value.IsObject()) {
        recursiveAdd(pref, arr, m.value, alloc);
      }
    }
  }
}
