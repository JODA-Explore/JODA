...
bool joda::query::RegexProvider::isConst() const { return false; }

RJValue joda::query::RegexProvider::getAtomValue(
    const RapidJsonDocument& json, RJMemoryPoolAlloc& /*alloc*/) const {
  DCHECK(isAtom()) << "Did not check for atom before calling";
  std::string toCheck;
  RJValue val;
  val.SetBool(getParamString(toCheck, params[0], json) &&
              std::regex_search(toCheck, regex));
  return val;
}

const RJValue* joda::query::RegexProvider::getValue(
    const RapidJsonDocument& /*json*/, RJMemoryPoolAlloc& /*alloc*/) const {
  DCHECK(!isAtom()) << "Did not check for atom before calling";
  return nullptr;
}
...
joda::query::RegexProvider::duplicate() const {
  return std::make_unique<joda::query::RegexProvider>(duplicateParameters());
}

std::string joda::query::RegexProvider::getName() const { return "REGEX"; }

joda::query::IValueType joda::query::RegexProvider::getReturnType() const {
  return IV_Bool;
}

joda::query::RegexProvider::RegexProvider(
