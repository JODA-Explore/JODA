...
joda::query::INListProvider::duplicate() const {
...
RJValue joda::query::INListProvider::getAtomValue(
  ...
  if (checkList == nullptr || checkList->IsNull()) {
    return RJValue();
  }
...
const RJValue* joda::query::INListProvider::getValue(
    const RapidJsonDocument& /*json*/, RJMemoryPoolAlloc& /*alloc*/) const {
  assert(!isAtom() && "Did not check for atom before calling");
  return nullptr;
}

joda::query::INListProvider::INListProvider(
...
std::string joda::query::INListProvider::getName() const { return "IN"; }
