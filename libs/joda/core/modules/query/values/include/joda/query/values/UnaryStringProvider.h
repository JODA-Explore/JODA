 ...
 *  inline static RJValue calculate(std::string &&val, RJMemoryPoolAlloc &alloc) {  return <result>; };
 * };
 * @endcode
 */
template<class Calc>
class UnaryStringProvider : public joda::query::IValueProvider {
 public:
  explicit UnaryStringProvider(std::vector<std::unique_ptr<IValueProvider>> &&parameters)
...
struct UnaryLenCalculationFunction {
  ...
  inline static RJValue calculate(std::string &&val, RJMemoryPoolAlloc &alloc) { return RJValue(val.size()); };
};
...
struct UnaryRtrimCalculationFunction {
  ...
  inline static RJValue calculate(std::string &&val, RJMemoryPoolAlloc &alloc) {
    val.erase(std::find_if(val.rbegin(), val.rend(), [](int ch) {
      return !std::isspace(ch);
    }).base(), val.end());
    return RJValue(val.c_str(), alloc);
  };
...
template
class UnaryStringProvider<UnaryLenCalculationFunction>;

template
class UnaryStringProvider<UnaryUpperCalculationFunction>;

template
class UnaryStringProvider<UnaryLowerCalculationFunction>;

template
class UnaryStringProvider<UnaryLtrimCalculationFunction>;

template
class UnaryStringProvider<UnaryRtrimCalculationFunction>;
}
