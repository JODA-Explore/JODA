...
bool joda::query::EqualizePredicate::isCompatible(
    joda::query::Predicate* other) {
...
void joda::query::EqualizePredicate::accept(
    class joda::query::PredicateVisitor& v) {
  v.visit(this);
}

joda::query::EqualizePredicate::EqualizePredicate(
  ...
  if (!this->lhs->equalizable() || !this->rhs->equalizable()) {
...
joda::query::EqualizePredicate::duplicate() {
  return std::make_unique<joda::query::EqualizePredicate>(
      lhs->duplicate(), rhs->duplicate(), equal);
}
std::unique_ptr<joda::query::IValueProvider>&
joda::query::EqualizePredicate::getLhs() {
  return lhs;
}
std::unique_ptr<joda::query::IValueProvider>&
joda::query::EqualizePredicate::getRhs() {
  return rhs;
}
bool joda::query::EqualizePredicate::isEqual() const { return equal; }
