...
void joda::query::StaticEvalVisitor::visit(joda::query::AndPredicate* e) {
  ...
  if (lhsConst && rhsConst) {  // If both Const: X && Y => True/False
    pred = std::make_unique<joda::query::ValToPredicate>(
        std::make_unique<BoolProvider>(lhsTrue && rhsTrue));
    ...
    pred = std::make_unique<joda::query::AndPredicate>(std::move(lhs),
                                                       std::move(rhs));
  }
  DCHECK(pred != nullptr);
}
...
void joda::query::StaticEvalVisitor::visit(joda::query::OrPredicate* e) {
  ...
  if (lhsConst && rhsConst) {  // If both Const: X || Y => True/False
    ...
    pred = std::make_unique<joda::query::OrPredicate>(std::move(lhs),
                                                      std::move(rhs));
  }
  DCHECK(pred != nullptr);
}
...
void joda::query::StaticEvalVisitor::visit(joda::query::ComparePredicate* e) {
  ...
  if (lhs->isConst() && rhs->isConst()) {
    pred = std::make_unique<joda::query::ValToPredicate>(
        e->check(RapidJsonDocument()));
    LOG(INFO) << lhs->toString() << (e->isGreater() ? ">" : "<")
              << (e->isInclude() ? "=" : "") << rhs->toString() << " => "
              << (e->check(RapidJsonDocument()) ? JODA_TRUE_STRING
                                                : JODA_FALSE_STRING);
    changed = true;
  } else {
    pred = std::make_unique<joda::query::ComparePredicate>(
        std::move(lhs), std::move(rhs), e->isGreater(), e->isInclude());
  }
  DCHECK(pred != nullptr);
}
...
joda::query::StaticEvalVisitor::getPred() {
  DCHECK(pred != nullptr);
