...
joda::query::NegatePredicate::NegatePredicate(std::unique_ptr<Predicate> pred) : pred(std::move(pred)) {
}
...
void joda::query::NegatePredicate::accept(struct joda::query::PredicateVisitor &v) {
    v.visit(this);
}

void joda::query::NegatePredicate::subAccept(struct joda::query::PredicateVisitor &v) {
    pred->accept(v);
}
