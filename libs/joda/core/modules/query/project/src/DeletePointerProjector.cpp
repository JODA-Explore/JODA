...
joda::query::DeletePointerProjector::DeletePointerProjector(const std::string &to) : IProjector(to) {
...
RJValue joda::query::DeletePointerProjector::getVal(const RapidJsonDocument &json, RJMemoryPoolAlloc &alloc) {
  return RJValue();
}

void joda::query::DeletePointerProjector::project(const RapidJsonDocument &json, RJDocument &newDoc, bool view) {
  if (view && ptr.GetTokenCount() > 0) { //Is View and not root node?
    ...
    if (vValue != nullptr) { //View does not contain the parent
      ...
      if (pValue != nullptr) {//If in original document parent is an object, create empty object in view
        if (pValue->IsObject()) { parent.Set(newDoc, RJValue(rapidjson::kObjectType)); }
      }
    }
