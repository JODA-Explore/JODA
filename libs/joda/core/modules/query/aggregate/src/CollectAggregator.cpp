...
RJValue joda::query::CollectAggregator::terminate(RJMemoryPoolAlloc &alloc) {
  return std::move(*list);
}

std::unique_ptr<joda::query::IAggregator> joda::query::CollectAggregator::duplicate() const {
  return std::make_unique<CollectAggregator>(toPointer,duplicateParameters());
}

void joda::query::CollectAggregator::accumulate(const RapidJsonDocument &json, RJMemoryPoolAlloc &alloc) {
  const RJValue* val;
  RJValue val_;
...
joda::query::CollectAggregator::CollectAggregator(const std::string &toPointer,
                                                  std::vector<std::unique_ptr<IValueProvider>> &&params) : IAggregator(toPointer,
                                                                                                          std::move(params))
                                                                                                          {
