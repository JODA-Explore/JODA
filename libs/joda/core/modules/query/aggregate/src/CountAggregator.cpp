...
joda::query::CountAggregator::duplicate() const {
  return std::make_unique<CountAggregator>(toPointer, duplicateParameters());
}

joda::query::CountAggregator::CountAggregator(
    const std::string& toPointer,
    std::vector<std::unique_ptr<IValueProvider>>&& params)
    : IAggregator(toPointer, std::move(params)) {
  if (this->params.size() > 1) {
    throw WrongParameterCountException(this->params.size(), 1, getName());
  }
}

void joda::query::CountAggregator::accumulate(const RapidJsonDocument& json,
