...
class NumberAggregatorTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    referenceAgg =
        std::make_unique<joda::query::NumberAggregator<T>>(args, std::move(x));
  }
...
using AggregatorFuncTypes = testing::Types<
    joda::query::MinAggregatorFunc, joda::query::MaxAggregatorFunc,
    joda::query::SumAggregatorFunc, joda::query::AverageAggregatorFunc>;

template <>
double NumberAggregatorTest<joda::query::MinAggregatorFunc>::aggVector(
...
template <>
double NumberAggregatorTest<joda::query::MaxAggregatorFunc>::aggVector(
    const std::vector<double>& vec) {
...
template <>
double NumberAggregatorTest<joda::query::SumAggregatorFunc>::aggVector(
    const std::vector<double>& vec) {
  return std::accumulate(vec.begin(), vec.end(), (double)0);
}

template <>
double NumberAggregatorTest<joda::query::AverageAggregatorFunc>::aggVector(
    const std::vector<double>& vec) {
  if (!vec.empty())
...
TYPED_TEST(NumberAggregatorTest, Construct) {
  ...
  arguments.emplace_back(
      std::make_unique<joda::query::PointerProvider>(fromPtr));
  std::unique_ptr<joda::query::IAggregator> agg;
  EXPECT_NO_THROW(
      agg = std::make_unique<joda::query::NumberAggregator<TypeParam>>(
          toPtr, std::move(arguments)););
...
TYPED_TEST(NumberAggregatorTest, TooFewParams) {
  ...
  std::vector<std::unique_ptr<joda::query::IValueProvider>> arguments;
  std::unique_ptr<joda::query::IAggregator> agg;
  EXPECT_THROW(agg = std::make_unique<joda::query::NumberAggregator<TypeParam>>(
                   toPtr, std::move(arguments));
...
TYPED_TEST(NumberAggregatorTest, Duplicate) {
  ...
  EXPECT_STREQ(this->referenceAgg->toString().c_str(),
               dupe->toString().c_str());
}
