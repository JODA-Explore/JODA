...
void QueryCache::addQueryResult(std::unique_ptr<CacheEntry>&& ce) {
  ...
  if (cache != queryMap.end()) {
    ...
    return;
  } else { //If it does not exist, also add it
...
std::shared_ptr<const CacheEntry::CacheIndex> QueryCache::getBestCache(const std::shared_ptr<joda::query::Predicate> &pred) {
  if (queryMap.empty()) return nullptr;//No need to stringify if cache is empty
  joda::query::ToStringVisitor stringify;
...
bool QueryCache::cacheAvailable(const std::shared_ptr<joda::query::Predicate> &pred) {
  if (queryMap.empty()) return false;//No need to stringify if cache is empty
  joda::query::ToStringVisitor stringify;
  pred->accept(stringify);
  auto str = stringify.popString();
