...
class ReaderParser {
 ...
 private:
  void startParser(const std::vector<std::unique_ptr<IImportSource>> &sources,
                   ...
                   std::unordered_set<ReaderParser::ParserID,
  ...
  inline std::unique_ptr<IOThreadPool<Reader>> createReader(
      RQueueCont &rqueue, PQueueCont &pqueue,
      std::unordered_set<ReaderParser::ReaderID,
                         boost::hash<ReaderParser::ReaderID>> &readerIds,
...
template <template <class...> class T, class... Args>
std::unique_ptr<IThreadUser> ReaderParser::createParserFromConfig(
    Args... args) {
  if (config::sim_measure == config::NO_SIMILARITY) {
    ...
    return std::make_unique<IOThreadPool<T<SimilarityScheduler<PathJaccard>>>>(
        args...);
  } else if (config::sim_measure == config::Sim_Measures::ATTRIBUTE_JACCARD) {
    return std::make_unique<
        IOThreadPool<T<SimilarityScheduler<AttributeJaccard>>>>(args...);
  } else {
...
auto ReaderParser::createReaderQueue(
    std::unordered_set<ReaderParser::ReaderID,
                       boost::hash<ReaderParser::ReaderID>> &readerIds) {
  std::pair<std::unique_ptr<typename Queue::queue_t>,
            std::unique_ptr<typename Queue::queue_t::ptok_t>>
      queue;
  for (const auto &readerId : readerIds) {
    if (readerId.first == Queue::getFlag()) {
      queue.first = Queue::getQueue(
          config::read_bulk_size * (std::thread::hardware_concurrency()), 0,
          std::thread::hardware_concurrency());
      queue.second =
          std::make_unique<typename Queue::queue_t::ptok_t>(queue.first->queue);
      break;
    }
  }
  return queue;
...
template <class Reader, class RQueueCont, class PQueueCont>
std::unique_ptr<IOThreadPool<Reader>> ReaderParser::createReader(
    RQueueCont &rqueue, PQueueCont &pqueue,
    std::unordered_set<ReaderParser::ReaderID,
                       boost::hash<ReaderParser::ReaderID>> &readerIds,
    std::vector<size_t> &threadIDs) {
  ...
  for (const auto &readerId : readerIds) {
    if (readerId.first == Reader::getIQueueFlags() &&
        readerId.second == Reader::getOQueueFlags()) {
      DCHECK(rqueue.first != nullptr)
      ...
      DCHECK(std::get<0>(pqueue) != nullptr)
          << "Output queue missing for " << ReaderName;
      DCHECK((ParserFlag)std::get<0>(pqueue)->getFlag() ==
             Reader::getOQueueFlags())
          << "Output queue type mismatching for " << ReaderName;

      auto tmp = std::make_unique<IOThreadPool<Reader>>(
          rqueue.first.get(), std::get<0>(pqueue).get(), config::readingThreads,
          nullptr);
