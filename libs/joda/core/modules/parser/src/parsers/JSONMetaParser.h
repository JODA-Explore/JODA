 ...
 * Instead, only the metadata of their origin required for actual parsing is used.
 * This parser is useful for parsing in memory-rescricted environments
...
class JSONMetaParser : public IWorkerThread<JsonTextParserQueue,JsonContainerQueue,size_t>  {
 public:
  JSONMetaParser(IQueue *iqueue, OQueue *oqueue, size_t contSize): IWorkerThread(iqueue,oqueue,contSize), contSize(this->conf) {
    ...
    while (shouldRun) {
      if (!iqueue->isFinished()) {
        ...
        for (auto &text : buff) {
          ...
          sched.scheduleDocument(cont,nullptr, std::move(text.first), text.second.size());
