...
/**
 * Interface for classes reading documents and commiting their contents into a
 * queue The reader and parser flags describe which kind of data can be read and
 * what is returned.
...
class IReader : public IThreadUser {
 public:
  ...
  /**
   * Creates a new reader with the given input and output queue and a maximum
   * number of threads to use during reading
   * @param rqueue Input queue
   * @param pqueue Output queue
   ...
   */
  /**
   * Reads everything that is passed through the queue, and outputs it into the
   * output queue
   */
  virtual void read() { read(1); }

  /**
   * Reads a sample of everything that is passed through the queue, and outputs
...
IReader<R_FLAGS, P_FLAGS>::IReader(std::unique_ptr<IReader::RQueue>& rqueue,
  ...
  DLOG(INFO) << "Started IReader<" << JODA_FLAG_T(R_FLAGS) << ","
             << JODA_FLAG_T(P_FLAGS) << "> with " << maxThreads << " threads";
}
