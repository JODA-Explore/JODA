...
joda::docparsing::JSONFileSource::JSONFileSource(const std::string &filePath, const double sample) : JSONFileSource(filePath, false, sample) {}

void joda::docparsing::JSONFileSource::feedSources(JODA_READER_QUEUE<JODA_JSON_FILE_LINESEPERATED_READER_FLAG>::queue_t &queue,
...
void joda::docparsing::JSONFileSource::feedSources(JODA_READER_QUEUE<JODA_JSON_FILE_BEAUTIFIED_READER_FLAG>::queue_t &queue,
...
const std::string joda::docparsing::JSONFileSource::toString() {
  return "File: " + filePath + "; Separated: " + std::to_string(lineSeparated) + "; Sample: " + std::to_string(sample);
}
...
const std::string joda::docparsing::JSONFileSource::toQueryString() {
  ...
  if (lineSeparated) separated = " LINESEPARATED";
  return "FROM FILE \"" + filePath + "\"" + separated + sampleString;
}

joda::docparsing::JSONFileSource::JSONFileSource(const std::string &filePath, bool lineSeparated, const double sample) : lineSeparated(
    lineSeparated), filePath(filePath), sample(sample),
                                                                                                                         IImportSource(lineSeparated
