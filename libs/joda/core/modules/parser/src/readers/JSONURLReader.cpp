...
joda::docparsing::JSONURLReader::JSONURLReader(IQueue *iqueue, OQueue *oqueue, void* ptr) : IWorkerThread(iqueue, oqueue, nullptr) {
  ...
  while (shouldRun) {
    if (!iqueue->isFinished()) {
      ...
      try{
        ...
        auto stream = std::make_unique<Concurrency::streams::async_istream<char>>(resp.get().body().streambuf());
