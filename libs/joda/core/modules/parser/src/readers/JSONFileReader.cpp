...
joda::docparsing::JSONFileReader::JSONFileReader(IQueue *iqueue, OQueue *oqueue, void* ptr) : IWorkerThread(iqueue, oqueue, nullptr) {
...
void joda::docparsing::JSONFileReader::work() {
  ...
  while (shouldRun) {
    if (!iqueue->isFinished()) {
      ...
      if (fd == -1) {
        LOG(ERROR) << "Could not open \"" << file << "\" for reading (" << strerror(errno) << ").";
        continue;
      }
      //Get file stats
      struct stat fs;
      if (fstat(fd, &fs) == -1) {
        LOG(ERROR) << "Could not stat \"" << file << "\" for reading (" << strerror(errno) << ").";
        close(fd);
        continue;
      }

      posix_fadvise(fd, 0, 0, POSIX_FADV_SEQUENTIAL); // announce the desire to sequentialy read this file
      // silent error handling - weak error

      //Mmap file
      char *buf = static_cast<char *>(mmap(0, static_cast<size_t>(fs.st_size), PROT_READ, MAP_SHARED, fd, 0));
      if (buf == MAP_FAILED) {
        LOG(ERROR) << "Could not memoryServer mode did quit on every input map file \"" << file << "\" ("
                   << strerror(errno) << ").";
        close(fd);
        continue;
      }
      ...
      while ((end = static_cast<char *>(memchr(begin, '\n', static_cast<size_t>(buff_end - begin)))) != nullptr) {
        std::string line(begin, end);

        //Check sample
        if (!(sfile.sample < 1.0 && ((double) (rand() % 100)) / 100.0 >= sfile.sample)) { //Sampling
          //line_end = line_start+line.length()+1;
          buff.emplace_back(std::make_unique<FileOrigin>(fileID, begin - buf, end - buf + 1, index), std::move(line));
