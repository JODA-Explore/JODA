...
struct loadAction<fromURLKW> {
  ...
  static void apply(const Input &in, loadState &state) {
    ...
    throw tao::pegtl::parse_error("This program was compiled without 'FROM URL' support.", in);
#endif
    ...
    if (!state.loadFile.empty()) {
      switch (state.source){
        case NO_SOURCE:
          ...
          break;
        case FILE_SOURCE:
          state.sources.emplace_back(std::make_unique<docparsing::JSONFileSource>(state.loadFile, state.lineSeperated, state.sampleNum));
          break;
        case DIRECTORY_SOURCE:
          state.sources.emplace_back(std::make_unique<docparsing::JSONFileDirectorySource>(state.loadFile, state.lineSeperated, state.sampleNum));
          break;
        case URL_SOURCE:state.sources.emplace_back(std::make_unique<docparsing::JSONURLSource>(state.loadFile));
          break;
      }
