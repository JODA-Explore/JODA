...
struct chooseState {
  ...
  inline void success(const Input &in, queryState &qs) {
    ...
    assert(ptr.empty());
    if (preds.empty()) {
      std::unique_ptr<joda::query::Predicate> pred =
    ...
    if (preds.top().second.empty()) {
      std::unique_ptr<joda::query::Predicate> t =
          std::make_unique<joda::query::ValToPredicate>(true);
      preds.top().second.push_back(std::move(t));
    }
    assert(preds.top().second.size() == 1);
    std::unique_ptr<joda::query::Predicate> tmp =
        std::move(preds.top().second[0]);
  ...
  predStack preds{};
  std::pair<std::unique_ptr<joda::query::IValueProvider>,
