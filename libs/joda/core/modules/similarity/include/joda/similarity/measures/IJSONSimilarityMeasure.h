...
struct similarityRepresentation {
  static_assert(std::is_base_of<IJSONSimilarityMeasure, T>::value,
                "T must derive from IJSONSimilarityMeasure");
  typedef void *Representation;
  /**
   * Returns the internal representation needed to calculate a similarity
   * measure with T
   * @param lhs The document to transform into the representation
   * @return The representation of Document
   */
  Representation getRepresentation(const RJDocument &lhs) { return nullptr; }
   ...
   * @return  The representation of Document
   */
  Representation getRepresentation(const std::string &lhs) { return nullptr; }
