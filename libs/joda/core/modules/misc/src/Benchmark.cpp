...
Benchmark::Benchmark() : valid(config::benchmark), local(true) {
  if (valid) {
...
Benchmark::Benchmark(const std::string &file) : valid(false), benchfile(file) {
...
void Benchmark::parseBenchfile() {
  ...
  if (!benchDoc.HasParseError() || benchDoc.GetParseError() == rapidjson::ParseErrorCode::kParseErrorDocumentEmpty) {
    if (benchDoc.IsNull()) benchDoc.SetArray();
    valid = true;
    local = false;
  } else {
    valid = true;
    local = true;
    LOG(ERROR) << "Could not parse benchfile: " << rapidjson::GetParseError_En(benchDoc.GetParseError());
  }
}
...
void Benchmark::finishLine() {
  if (valid) {
    ...
    versionInfo.AddMember("Version", JODA_VERSION_STRING, benchDoc.GetAllocator());
    versionInfo.AddMember("Build", JODA_GIT_REVISION, benchDoc.GetAllocator());
    versionInfo.AddMember("Build Time", JODA_BUILD_TIME, benchDoc.GetAllocator());
...
void Benchmark::addThread(double bloom, double select, double project, double agg, double copy, double serialize) {
  ...
  if (bloom > 0) thread.AddMember("Bloom", RJValue(bloom), benchDoc.GetAllocator());
  if (select > 0) thread.AddMember("Select", RJValue(select), benchDoc.GetAllocator());
  if (project > 0) thread.AddMember("Project", RJValue(project), benchDoc.GetAllocator());
