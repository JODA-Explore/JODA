...
MemoryUtility::MemoryT MemoryUtility::MemorySize::convertToBytes(const MemoryT xbytes, const int powerOfThousand){
  return std::floor(xbytes * std::pow(1024,powerOfThousand));
}

MemoryUtility::MemoryT MemoryUtility::MemorySize::convertToBytesSI(const MemoryT xbytes, const int powerOfThousand){
...
MemoryUtility::MemoryT MemoryUtility::MemorySize:: getBase(const int base) const{
  return std::floor(bytes / std::pow(1024,base));
}

MemoryUtility::MemoryT MemoryUtility::MemorySize:: getBaseSI(const int base) const{
  return std::floor(bytes / std::pow(1000,base));
}

double MemoryUtility::MemorySize::getFBase(const int base) const{
  return ((double)bytes) / std::pow(1024,base);
}

double MemoryUtility::MemorySize::getFBaseSI(const int base) const{
  return ((double)bytes) / std::pow(1000,base);
}

std::string MemoryUtility::MemorySize::getHumanReadable() const{
  double remainingBytes = bytes;
  int suffix = 0;
  std::vector<std::string> suffixes = {"Bi","KBi","MBi","GBi","TBi","PBi","EBi"};
  while(remainingBytes > 1024 && suffix < suffixes.size()-1){
    remainingBytes /= 1024;
    suffix++;
  }
  std::ostringstream ss;
  ss << std::fixed << std::setprecision(2) << remainingBytes << " " << suffixes[suffix];
...
MemoryUtility::MemorySize MemoryUtility::procRamUsage() {
  ...
  fclose(file);
  return MemoryUtility::MemorySize::convertToBytes(result,MemoryUtility::MemorySize::KB);
}
...
double MemoryUtility::getFreeRam() {
  return 1 - ((double) sysRamUsage().getBytes()) / totalRam().getBytes();
}
