...
void RecurringTimer::stop() {
  totalDur += std::chrono::duration_cast<std::chrono::microseconds>(now()-startTime);
}

void RecurringTimer::log(const std::string &what, const std::string &part) const {
#ifdef JODA_TIMING_ENABLED
  std::string duration = "ms";
  double diff = std::chrono::duration_cast<std::chrono::milliseconds>(totalDur).count();
  if(diff > 1000){
    duration = "s";
    diff /= 1000;
  }
...
double RecurringTimer::durationSeconds() const {
  ...
  return 0;
#endif
  auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(totalDur).count();
