...
FILEID FileNameRepo::getNextID() { return ++currID; }
...
std::string FileNameRepo::getFile(FILEID id) {
  ...
  if (id == ProjectionFileName) {
    return "[PROJECTION]";
  }
  if (id == TMPObjectName) {
    return "[TEMPORARYOBJECT]";
  }
  LOG(ERROR) << "Tried to retrieve file belonging to id " << id
             << ". But id is not found in repository.";
  return "";
}
