...
void JoinParser::parse(const FileJoinManager& jm,
  ...
  auto parsingThreads = std::max(
      1ul, std::min(static_cast<ulong>(std::thread::hardware_concurrency()) / 4,
                    std::min(4ul, fileList.size())));

  readers.reserve(parsingThreads);
  for (size_t i = 0; i < parsingThreads; ++i) {
    readers.push_back(std::async(std::launch::async, [&] {
      auto cont = std::make_unique<JSONContainer>();
      while (filesParsed.load() < filesSent.load()) {
        // Get File
        std::string file;
        if (!files.try_dequeue(file)) {
          continue;
        }
        ...
        while (std::getline(infile, line)) {
          ...
          if (tmpDoc.HasParseError()) {
            LOG(ERROR) << std::string(rapidjson::GetParseError_En(
        ...
        cont->insertDoc(std::move(joinDoc),
  ...
  while (loaded != parsed) {
    ...
    LOG(INFO) << "Parsed/Read: " << std::to_string(parsed) << "/"
              << std::to_string(loaded)
              << " Reading threads: " << std::to_string(readers.size())
              << " Container Queue: "
              << std::to_string(contQueue.size_approx());
  ...
  archiver.join();
  LOG(INFO) << "Parsing stopped, used " << std::to_string(readers.size())
            << " threads for parsing.";
}
