...
bool FileJoinManager::createFile(const std::string &file, std::string val) {
  std::ofstream o(file);
  //TODO Replace '\n'
  o << "STR"<<'\n' << val << std::endl;
  o.close();
...
FileJoinManager::FileJoinManager(const std::string &name, std::unique_ptr<joda::query::IValueProvider> &&valProv): JoinManager(name, std::move(valProv)) {
  LOG(INFO) << "Registering join-directory";
  auto dir = joda::filesystem::DirectoryRegister::getInstance().getTMPDir(name);
  if(dir) baseDir = *dir;
  else throw joda::filesystem::DirectoryAlreadyInUseException();
}
...
bool FileJoinManager::appendDocToFile(const RapidJsonDocument &doc, const std::string &file) {
  std::ofstream o;
