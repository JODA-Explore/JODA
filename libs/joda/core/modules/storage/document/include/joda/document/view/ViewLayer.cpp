...
bool ViewLayer::pathOverwritten(const std::string &path) const {
  if (viewPaths == nullptr) return true;
  return std::find_if(viewPaths->begin(), viewPaths->end(), [&path](const std::string &p) {
...
ViewLayer::ViewLayer(const RJValue *doc,
    ...
    : doc(doc), viewPaths(viewPaths), previousLayer(previousLayer), struc(struc) {
}
...
void ViewLayer::getCursor(const std::string &ptr) {
  if (cursor != nullptr) {
    ...
    if (previousLayer != nullptr) previousLayer->getCursor("");
    if (doc == nullptr) return;

    if (previousLayer == nullptr) {
      cursor = std::make_unique<ViewCursor>(doc, nullptr, viewPaths);
    } else {
      cursor = std::make_unique<ViewCursor>(doc, previousLayer->cursor.get(), viewPaths);
    }
  }
  if (!ptr.empty()) {
...
RJValue const *ViewLayer::Get(const RJPointer &ptr, const std::string &ptrStr)  {
  if (doc != nullptr && pathOverwritten(ptrStr)) {
    ...
    if (previousLayer == nullptr) return nullptr;
    else return previousLayer->Get(ptr, ptrStr);
  }
}
...
bool ViewLayer::pointerIsOverwritten(const std::string &ptr) const {
  if (viewPaths == nullptr) return true;
  return std::find(viewPaths->begin(), viewPaths->end(), ptr) != viewPaths->end();
}

const VirtualObject *ViewLayer::getVO(const std::string &ptr) const {
  auto obj = objects.find(ptr);
  if (obj != objects.end()) return &obj->second;
  return nullptr;
}

void ViewLayer::lock() {
  if (previousLayer != nullptr) previousLayer->lock();
  mut.lock();
}

void ViewLayer::unlock() {
  if (previousLayer != nullptr) previousLayer->unlock();
  mut.unlock();
}

std::vector<ViewCursor::ViewObjectMember> ViewLayer::GetMembers(const std::string &currPath) {
  ...
  if (cursor->isBase(currPath)) {
    ...
    if (val != nullptr && val->IsObject()) {
      for (typename RJDocument::ConstMemberIterator m = val->MemberBegin(); m != val->MemberEnd(); ++m) {
        members.emplace_back(std::string(m->name.GetString(), m->name.GetStringLength()), &m->value);
      }
    }
    return members;
  } else {
    ...
    if (val != nullptr && val->IsObject()) { //Val is either object, or null if no changes are written to the delta tree
      members.erase(std::remove_if(members.begin(), members.end(), [&val, &currPath, this](auto &member) {
        auto tmpPath = currPath + "/" + member.name;
        return std::find(viewPaths->begin(), viewPaths->end(), tmpPath) != viewPaths->end() &&
            val->FindMember(member.name) == val->MemberEnd();
      }), members.end());
      for (typename RJDocument::ConstMemberIterator m = val->MemberBegin(); m != val->MemberEnd(); ++m) {
        auto name = std::string(m->name.GetString(), m->name.GetStringLength());
        auto iterator =
            std::find_if(members.begin(), members.end(), [&name](auto &member) { return member.name == name; });
        if (iterator == members.end()) {
          members.emplace_back(std::move(name), &m->value);
        } else {
          auto tmpPath = currPath + "/" + name;
          if (std::find(viewPaths->begin(), viewPaths->end(), tmpPath) != viewPaths->end()) {
