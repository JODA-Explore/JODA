...
const ViewStructure::KEY *ViewStructure::getKey(const ViewStructure::ID &id) const {
  auto it = id_key.find(id);
  if (it == id_key.end()) return nullptr;
  return it->second;
}
...
void ViewStructure::addMember(size_t i, ViewStructure::ID id, const RJValue *val) {
  DCHECK(i < members.size());
  members[i] = {id, val};
}

void ViewStructure::addMember(size_t i, ViewStructure::ID id, const VirtualObject *val) {
  DCHECK(i < members.size());
  members[i] = {id, val};
}
...
ViewStructure::ObjectMember::ObjectMember(ViewStructure::ID key, const RJValue *val)
    : key(key), val(val), obj(nullptr) {}

ViewStructure::ObjectMember::ObjectMember(ViewStructure::ID key, const VirtualObject *obj)
