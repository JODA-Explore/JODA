...
ViewCursor::ViewCursor(const RJValue *doc,
                       ViewCursor *prev, const std::vector<std::string> *viewPaths) : _stack({doc}),
                                                                                      previousCursor(std::move(prev)),
                                                                                      viewPaths(viewPaths) {
...
void ViewCursor::getOverwrites(const std::string &curr, std::vector<ViewCursor::ViewObjectMember> &previousOverwrites) {
  if (_stack.back() == nullptr || !_stack.back()->IsObject()) return;

  auto baseAttPath = curr + "/";
  auto baseAttSize = baseAttPath.size();
  if (viewPaths == nullptr) return;
  for (const auto &viewpath : *viewPaths) {
    if (viewpath.find(curr) == 0) {
      ...
      auto
          f = std::find_if(previousOverwrites.begin(), previousOverwrites.end(), [&nameStr](const ViewObjectMember &m) {
        return m.name == nameStr;
      });
      if (f == previousOverwrites.end()) {
        auto it = _stack.back()->FindMember(nameStr);
        if (it == _stack.back()->MemberEnd()) {
          previousOverwrites.emplace_back(std::move(nameStr), nullptr); //Add removed Value
        } else {
          previousOverwrites.emplace_back(std::move(nameStr), &it->value); //Add Changed
        }
      }
...
void ViewCursor::unfollow() {
  DCHECK(!_stack.empty());
  _stack.pop_back();
  if (previousCursor != nullptr) previousCursor->unfollow();
}
...
void ViewCursor::reset() {
  nextDestination = nullptr;
  nextDestinationSet = false;
  while (_stack.size() > 1) _stack.pop_back();
  if (previousCursor != nullptr) previousCursor->reset();
...
bool ViewCursor::isBase(const std::string &currPointer) const {
  if (previousCursor == nullptr) return true;
  for (const auto &viewpath : *viewPaths) {
    if (currPointer.find(viewpath) == 0) return true;
  }
  return false;
}
...
bool ViewCursor::isShared(const std::string &currPointer) const {
  ...
  //Get Basecursor
  while (!baseCursor->isBase(currPointer))baseCursor = baseCursor->previousCursor;
  const ViewCursor *iCursor = this;
  while (iCursor != baseCursor->previousCursor) {
    for (const auto &viewPath : *iCursor->viewPaths) {
      if (viewPath.find(currPointer) == 0) return true;
    }
