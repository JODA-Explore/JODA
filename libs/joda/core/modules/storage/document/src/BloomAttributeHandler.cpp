...
bool BloomAttributeHandler::RawNumber(const char *str, rapidjson::SizeType len, bool copy) {
  popStack();
  incArray();
  return true;
}

bool BloomAttributeHandler::String(const char *ch, rapidjson::SizeType type, bool b) {
  popStack();
  incArray();
  return true;
}
...
bool BloomAttributeHandler::Key(const char *str, rapidjson::SizeType len, bool copy) {
  addStack(str, len);
  addPath();
  return true;
}
...
BloomAttributeHandler::BloomAttributeHandler() : stack(), paths() {
...
void BloomAttributeHandler::stopArray() {
  arrStack.pop_back();
}
