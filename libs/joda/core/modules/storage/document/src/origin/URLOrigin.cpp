...
std::string URLOrigin::toString() const {
  return std::string("URL: ") + std::to_string(id) + ":" + std::to_string(start) + "-"
      + std::to_string(end);
}
...
URLOrigin::URLOrigin(FILEID url, long start, long end, int index) : IDPositionOrigin(url, start, end, index) {}
