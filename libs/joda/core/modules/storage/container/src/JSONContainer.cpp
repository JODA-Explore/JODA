...
void JSONContainer::insertViewDoc(
...
DocumentCostHandler JSONContainer::createTempViewDocs(const DocIndex &ids,
  ...
  if (proj.empty() && setProj.empty()) {
    for (size_t i = 0; i < docs.size(); ++i) {
      ...
      tmpCont->insertDoc(std::move(newDoc), docs[i].doc.getOrigin()->clone(), i);
    }
  } else {
    bool used = false;
    for (size_t j = 0; j < docs.size(); ++j) {
      ...
      if (setProj.empty()) {
        costHandler.checkDocument(*newDoc);
        if (valid)tmpCont->insertViewDoc(std::move(newDoc), j);
      } else {
        std::vector<std::unique_ptr<RJDocument>> newDocs;
        newDocs.push_back(std::move(newDoc));
        for (auto &&sp : setProj) {
    ...
    //Add Paths
    tmpCont->materializedAttributes.reserve(tmpCont->materializedAttributes.size() + proj.size() + setProj.size());
...
std::unique_ptr<JSONContainer> JSONContainer::createViewFromContainer(const DocIndex &ids,
                                                                      ...
                                                                      const std::vector<
                                                                          std::unique_ptr<joda::query::ISetProjector>> &setProj) {
  DCHECK(config::enable_views && (((!proj.empty() && proj.front()->getType() == joda::query::PointerCopyProject::allCopy))
...
size_t JSONContainer::estimatedSize() const {
  ...
  if (viewStruc != nullptr) size += viewStruc->estimateSize();
...
std::vector<std::unique_ptr<RJDocument>> JSONContainer::getRaw(unsigned long start, unsigned long end) {
  std::vector<std::unique_ptr<RJDocument>> ret;
  if(docs.empty()) return ret;
  ScopedRef useCont(this, false);
  reparseSubset(start,end);
...
std::vector<std::unique_ptr<RJDocument>> JSONContainer::getRaw(const DocIndex &ids, RJMemoryPoolAlloc &alloc) {
  std::vector<std::unique_ptr<RJDocument>> ret;
  if (docs.empty()) return ret;
  ScopedRef ref(this);
...
void JSONContainer::insertDoc(
    std::unique_ptr<RJDocument> &&doc,
    std::unique_ptr<IOrigin> &&origin, size_t baseIndex) {
...
JSONContainer::~JSONContainer() {
  while (!subContainers.empty()) {
    subContainers.back()->materializeView(); //This calls RemoveSubContainer => subcontainer will be smaller
  }
  if (isView()) {
...
void JSONContainer::materializeAttributesIfRequired(const std::vector<std::string> &atts) {
  /*
   * Shared objects are objects of which at least one attribute is overwritten by the view.
   * Thereby we materialize all attributes for which materializedAttributes contains an entry which is prefixed by the att
   */
  if (!isView()) return;
  ...
  for (const auto &att : atts) {
    if (att.empty()) return materializeView(); //Materialize whole document
    if (std::find(materializedAttributes.begin(), materializedAttributes.end(), att)
        != materializedAttributes.end())
      continue;
    ...
    if (!found) subatts.emplace_back(att); //If not prefixed in this view, maybe in one below
  }
  if (!subatts.empty()) baseContainer->materializeAttributesIfRequired(subatts);
  if (!newatts.empty()) materializeAttributes(newatts);
}
...
void JSONContainer::materializeAttributes(const std::vector<std::string> &atts) {
  ...
  for (const auto &att : atts) {
    ...
    for (auto i = materializedAttributes.begin(); i != materializedAttributes.end();) {
...
void JSONContainer::removeSubContainer(JSONContainer *cont) {
  ...
  if (it != subContainers.end()) subContainers.erase(it);
}

void JSONContainer::setViews() {
  if (viewsComputed || !config::enable_views) return;
  if (viewStruc == nullptr) viewStruc = std::make_unique<ViewStructure>(size());
  if (!isView()) {
...
bool JSONContainer::useViewBasedOnSample(const DocIndex &ids, const std::vector<
    std::unique_ptr<joda::query::IProjector>> &proj, const std::vector<
    std::unique_ptr<joda::query::ISetProjector>> &setProj) {
  auto count = std::count(ids.begin(), ids.end(), true);
  if (count <= 100) return true; //too small count, makes no sense
  auto sample_count_f = count * 0.01;
  auto sample_num = std::max((size_t) 1, static_cast<size_t>(sample_count_f));
  ...
  if(proj.empty() && setProj.empty()){
    forAll([&defaultCost](const RapidJsonDocument &d) {
...
size_t JSONContainer::parsedSize() const {
  if (lastParsedSize != 0) return lastParsedSize;
  else return estimatedSize();
}
