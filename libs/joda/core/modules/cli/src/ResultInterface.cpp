...
void joda::cli::ResultInterface::updateContent() {
  ...
  werase(results);
  if(maxResultI-minResultI<maxResults) maxResultI = std::min(storageSize,minResultI+maxResults-1);
  if(cache.empty()){
    initCache();
  }
...
void joda::cli::ResultInterface::addCache(size_t i, bool back) {
  ...
  entry.front() = entry.front().substr(0, static_cast<unsigned long>(maxLineLen));
  if (entry.size() == 1){
    if(back){
