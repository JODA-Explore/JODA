...
char** joda::cli::CLICompletion::completer(const char* text, int start,
                                           int /*end*/) {
  std::string line(rl_line_buffer, 0,
                   start);  // Get line until before the currently typed word
  ...
  if (start == 0) {  // Currently typed word is at start of buffer
    ...
    return rl_completion_matches(
        text, CLICompletion::basic_command_completer);  // Return all commands
  }
  // Load Variable Context
  std::regex loadVar(R"reg(^\s*LOAD\s*(?!>[[:alpha:]])$)reg");
  if (std::regex_search(line,
                        loadVar)) {  // Only "load" before currently typed word
    //   DLOG(INFO) << "storage_completer";
    return rl_completion_matches(
        text, CLICompletion::storage_completer);  // Return storage names
  }

  // Expand Files
  std::regex loadFiles(
      R"reg(^\s*LOAD\s*(?:[[:alpha:]]*)\s*FROM\s*FILE(?:S?)\s*"(?!.*"))reg");
...
char* joda::cli::CLICompletion::basic_command_completer(const char* text,
                                                        int state) {
  if (state == 0) {
...
char* joda::cli::CLICompletion::basic_query_completer(const char* text,
                                                      int state) {
  if (state == 0) {
...
void joda::cli::CLICompletion::init_match(const std::vector<std::string>& cmds,
                                          const char* text) {
  // Clear matches
  matches.clear();
  match_index = 0;
