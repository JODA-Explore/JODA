...
void joda::cli::CLI::start(const std::vector<std::string> &onceQueries) {
  ...
  if (isatty(STDIN_FILENO)) {
    ...
    if(config::persistent_history){
      if(config::history_file.empty()) LOG(WARNING) << "Persistent history enabled, but no history file provided.";
      else{
        DLOG(INFO) << "Reading history file \"" <<config::history_file<<"\"";
        auto err = read_history(config::history_file.c_str());
        if(err != 0){
          if(err == 2) DLOG(INFO) << "History file does not exist. Will be created on exit.";
          else LOG(ERROR) << "Error " << err << " while reading history file.";
        }else{
  ...
  if (isatty(STDIN_FILENO)) {
    ...
    if (err != 0) LOG(ERROR) << "Error " << err << " while writing history file.";
  }
}
...
void joda::cli::CLI::toggleCache() {
  ...
  print(std::string() + "Cache is now turned " + (config::queryCache ? "on" : "off"));
}
...
void joda::cli::CLI::listSources() {
  ...
  for (auto &storage : StorageCollection::getInstance().getStorages()) {
    tmp += storage->getName() + " (" + std::to_string(storage->size()) + ", " +
...
void joda::cli::CLI::listResults() {
  std::string tmp;
  tmp ="----Results----\n" ;
  for (auto &storage : StorageCollection::getInstance().getTemporaryStorages()) {
    tmp +=  storage->getName() + " (" + std::to_string(storage->size()) + ", " +
              MemoryUtility::MemorySize(storage->estimatedSize()).getHumanReadable() + ")\n" ;
  }
  tmp +=  "---------------" ;
...
void joda::cli::CLI::executeNonInteractiveQuery(std::shared_ptr<query::Query> &query, bool printResult) {
  if (query != nullptr) {
...
std::shared_ptr<JSONStorage> joda::cli::CLI::checkResult(unsigned long resultId) {
  if (resultId >= JODA_STORE_VALID_ID_START) {
    ...
    if (result == nullptr) {
      std::cerr << "Could not execute query, check log for additional information" << std::endl;
    } else {
    ...
    if (resultId == JODA_STORE_EXTERNAL_RS_ID) {
      ...
      std::cout << "Skipped query because of an error, see log for more info." << std::endl;
...
void joda::cli::CLI::executeQuery(std::shared_ptr<query::Query> &query, bool printResult) {
  ...
  if (query != nullptr) {
    ...
    try {
      ...
      while(resultTaks.wait_for(std::chrono::milliseconds(100))!=std::future_status::ready){
        ...
        if(parsing) status1 = "Parsed Docs/Conts: " + std::to_string(stats.parsedDocs)+"("+std::to_string(static_cast<size_t>(parsedDocs_s))+"/s)/"+std::to_string(stats.parsedConts)+"("+std::to_string(static_cast<size_t>(parsedConts_s))+"/s)";
...
joda::cli::CLI::CLI() : bench(config::benchfile) {
  checkTerminal();
}

void joda::cli::CLI::benchmarkQuery(std::shared_ptr<query::Query> q, const std::string &name) {
  QueryPlan plan(q);
...
void joda::cli::CLI::benchmarkQuery(std::vector<std::shared_ptr<query::Query>> qs, const std::string &name) {
  auto index = 1;
  for (auto &&q : qs) {
...
void joda::cli::CLI::setSimpleMode(bool simpleMode) {
  if(simpleMode){
    LOG(INFO) << "Using non interactive query processing due to terminal support.";
  }
...
void joda::cli::CLI::profileStart(const std::string &name) {
  if (profileRunning) std::cerr << "Profiler already running \n";
  else {
    auto n = name;
...
void joda::cli::CLI::profileStop() {
  if (!profileRunning) std::cerr << "No profiler running \n";
  else {
