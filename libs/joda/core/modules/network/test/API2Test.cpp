...
protected: 
    ...
    testing::AssertionResult respIsError(const std::string& errmsg = ""){
        RJDocument doc;
        doc.Parse(resp.body);
        if (doc.HasParseError()) return testing::AssertionFailure() << "Could not parse response";
        if (!doc.IsObject() || !doc.HasMember("error")) return testing::AssertionFailure() << "Response not an error";
        if(errmsg.empty()) return testing::AssertionSuccess(); 

        if (!doc["error"].IsString()) return testing::AssertionFailure() << "Error not a string message";
        if(std::string(doc["error"].GetString()) != errmsg ) return testing::AssertionFailure() << "Different error message. Expected '" << errmsg << +"' got '" <<doc["error"].GetString()<<"'";
        return testing::AssertionSuccess(); 
    }

    testing::AssertionResult respIsEqual(const std::string& expected){
        if (resp.body != expected) return testing::AssertionFailure() << "Unexpected Response. Expected '" << expected << +"' got '" <<resp.body<<"'";
        return testing::AssertionSuccess(); 
    }

};

TEST_F(API2Test, Version) {
    EXPECT_EQ(joda::network::apiv2::API::VERSION,2);
}
...
TEST_F(API2Test, DeleteNoParams) {
    ...
    EXPECT_TRUE(respIsError(joda::network::JodaMissingParameterException("name or result")));
}
...
TEST_F(API2Test, DeleteNegativeNumber) {
    ...
    ASSERT_NO_THROW(handler(req,resp));
    EXPECT_TRUE(respIsError(joda::network::JodaInvalidParameterException("result", "Cannot be parsed to unsigned long")));
}
...
TEST_F(API2Test, DeleteResult) {
    ...
    auto id = StorageCollection::getInstance().addTemporaryStorage(std::make_shared<JSONStorage>("Test"));
