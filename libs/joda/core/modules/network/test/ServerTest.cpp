...
class ServerTest : public testing::Test {
    ...
    void SetUp() override {
        ...
        serverThread = std::thread([&]() {
         ASSERT_TRUE(server.start("localhost",5632)) << "Could not start server, busy port?";
        });
...
TEST_F(ServerTest, Favicon) { 
    ...
    EXPECT_STREQ(response->body.c_str(),reinterpret_cast<const char *>(&joda::network::JODA_FAVICON[0]));
}
