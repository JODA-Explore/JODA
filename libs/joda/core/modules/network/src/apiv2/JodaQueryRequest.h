...
/**
* JodaQueryRequest is responsible for handling the execution of queries over the API
*/
class JodaQueryRequest {
 ...
 private:
  ...
  static void success_(unsigned long result,
  ...
  static RJDocument successDocument(unsigned long result,
                                    int64_t size,
...
class JodaQueryException : public JodaAPIException {
 public:
  ...
  explicit JodaQueryException(const std::string &what) : JodaAPIException("'Query' parameter invalid: " + what) {}

};

class JodaQueryUnkownErrorException : public JodaAPIException {
 public:
  JodaQueryUnkownErrorException() : JodaAPIException("A unknown error happened, check server logs for information.") {}

};

class JodaQuerySkippedException : public JodaAPIException {
 public:
  JodaQuerySkippedException() : JodaAPIException("The server skipped the query because of an error. Check logs") {}
