...
void joda::network::apiv2::JodaQueryRequest::registerEndpoint(const std::string &prefix, httplib::Server &server) {
...
std::vector<std::string> joda::network::apiv2::JodaQueryRequest::splitQueries(const std::string& query){
...
void joda::network::apiv2::JodaQueryRequest::query(const httplib::Request &req, httplib::Response &res) {
  ...
  for (const auto& q : querystrings){
    ...
    if (query == nullptr) throw (JodaQueryException(parser.getLastError()));
  ...
  if (result < JODA_STORE_VALID_ID_START) {
    if (result == JODA_STORE_EXTERNAL_RS_ID) {
      return export_(res);
    } else if (result == JODA_STORE_EMPTY_RS_ID) {
      return empty_(res);
...
void joda::network::apiv2::JodaQueryRequest::error_(unsigned long code, httplib::Response &res) {
  LOG(ERROR) << "Got invalid unknown special code as query result: " << code;
  throw (JodaQueryUnkownErrorException());
}

void joda::network::apiv2::JodaQueryRequest::skipped_(httplib::Response &res) {
...
void joda::network::apiv2::JodaQueryRequest::empty_(httplib::Response &res) {
  auto doc = successDocument(JODA_STORE_EMPTY_RS_ID,
                             -1,
                             "The query was executed on an empty collection.");
  JodaServer::sendResponse(doc, res);
}

RJDocument joda::network::apiv2::JodaQueryRequest::successDocument(unsigned long result,
                                                         int64_t size,
  ...
  if (!message.empty()) r.AddMember("message", message, r.GetAllocator());
...
void joda::network::apiv2::JodaQueryRequest::success_(unsigned long result,
...
void joda::network::apiv2::JodaQueryRequest::export_(httplib::Response &res) {
  auto doc = successDocument(JODA_STORE_EXTERNAL_RS_ID,
                             -1,
