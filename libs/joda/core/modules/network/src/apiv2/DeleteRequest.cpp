...
void joda::network::apiv2::DeleteRequest::registerEndpoint(const std::string &prefix, httplib::Server &server) {
...
void joda::network::apiv2::DeleteRequest::readParams(const httplib::Request &req, httplib::Response &res) {
  ...
  } else if (resultParam != req.params.end()) {
    unsigned long id = 0;
    try {
      if(strtod(resultParam->second.c_str(), NULL) < 0) throw std::out_of_range("Negative number");
      id = std::stoul(resultParam->second);
    } catch (std::exception &e) {
      throw (JodaInvalidParameterException("result", "Cannot be parsed to unsigned long"));
    }
...
void joda::network::apiv2::DeleteRequest::deleteSource(const httplib::Request &req,
...
void joda::network::apiv2::DeleteRequest::deleteResult(const httplib::Request &req, httplib::Response &res, unsigned long id) {
...
httplib::Server::Handler joda::network::apiv2::DeleteRequest::getHandler(const std::string prefix){
  auto fullEndpoint = prefix + endpoint;
  return [fullEndpoint](const httplib::Request &req, httplib::Response &res) {
    try {
      JodaServer::logExecutionTime(fullEndpoint, [&]() { readParams(req, res); });
