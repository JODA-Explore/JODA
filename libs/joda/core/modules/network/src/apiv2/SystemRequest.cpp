...
httplib::Server::Handler joda::network::apiv2::SystemRequest::getHandler(const std::string prefix){
  ...
  return [fullEndpoint](const httplib::Request &req, httplib::Response &res) {
    try {
      JodaServer::logExecutionTime(fullEndpoint, [&]() { sendSystemStats(req, res); });
...
void joda::network::apiv2::SystemRequest::registerEndpoint(const std::string &prefix, httplib::Server &server) {
  auto fullEndpoint = prefix + endpoint;
  server.Get(fullEndpoint.c_str(), getHandler(prefix));
  server.Post(fullEndpoint.c_str(), getHandler(prefix));
}

void joda::network::apiv2::SystemRequest::sendSystemStats(const httplib::Request &req, httplib::Response &res) {
  RJDocument ret(rapidjson::kObjectType);

  // Memory consumption
  RJValue memory(rapidjson::kObjectType);
  memory.AddMember("total", static_cast<uint64_t>(MemoryUtility::totalRam().getBytes()), ret.GetAllocator());
  memory.AddMember("used", static_cast<uint64_t>(MemoryUtility::sysRamUsage().getBytes()), ret.GetAllocator());
  memory.AddMember("joda", static_cast<uint64_t>(MemoryUtility::procRamUsage().getBytes()), ret.GetAllocator());
  memory.AddMember("allowed_memory", static_cast<uint64_t>(config::maxmemory), ret.GetAllocator());
  memory.AddMember("calculated_memory",
                   static_cast<uint64_t>(StorageCollection::getInstance().estimatedStorageSize()),
                   ret.GetAllocator());
