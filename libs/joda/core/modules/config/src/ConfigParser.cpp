...
const po::options_description ConfigParser::getCMDOptions()  {
  ...
  options.add_options()
      ...
      ("query,q", po::value<std::vector<std::string>>(), "Query to execute and then quit")
      ("queryfile,f", po::value<std::string>(), "Query file, containing lineseparated queries to execute and then quit")
      ("benchmark,b", "Runs the benchmark")
...
const po::options_description ConfigParser::getConfigOptions()  {
  ...
  options.add_options()
      //Directories
      ("data_dir,d", po::value<std::string>()->default_value("."), "Data dictionary for benchmarks (Default: Current directory)")
      ("tmpdir", po::value<std::string>(), "Directory for temporary files. (Default: OS chooses appropriate)")
      //Storage
      ...
      //Network
      ("port,p", po::value<unsigned int>()->default_value(5632), "Port for server mode")
      ("bind", po::value<std::string>()->default_value("localhost"), "Binding for server mode")
      //Misc
...
const po::options_description ConfigParser::getHiddenConfigOptions()  {
  ...
  if(env == nullptr) LOG(WARNING) << "Cannot get home directory";
  ...
  //Parsing
      ("parsing.read.bulk_size",po::value<unsigned int>()->default_value(50), "# of lines to read/send in bulk")
      ("parsing.read.reader",po::value<std::string>()->default_value("MMAP"), "Reader to choose per default (MMAP/FSTREAM)")
      ("parsing.parse.bulk_size",po::value<unsigned int>()->default_value(50), "# of lines to parse/send in bulk")
  //Similarity
      //("sim.min_cont_size",po::value<size_t>()->default_value(100),"Minimum container size (smaller will be merged)")
  ...
  //History
      ...
      ("history.persistent", po::value<bool>()->default_value(persistent), "Keep persistent history file?");
...
const po::variables_map ConfigParser::parseConfigs(int argc, char **argv)  {
  ...
  if(!configFile.empty()){
    ...
    po::store(po::parse_config_file<char>(configFile.c_str(), config_file_options,false), vm);
  }
...
void ConfigParser::setConfig(const po::variables_map &vm) {
  ...
  config::evictionStrategy = vm["storage.eviction_strategy"].as<config::EvictionStrategies>();

  //Benchmark
  if (vm.count("benchfile")) {
  ...
  //Containers
  config::JSONContainerSize = vm["storage.container.max_size"].as<unsigned int>();
  config::chunk_size = vm["storage.container.chunk_size"].as<double>();
  config::text_binary_mod = vm["storage.container.text_binary_mod"].as<double>();

  //Multithreading
  config::storageRetrievalThreads = vm["maxthreads"].as<unsigned int>();
  config::readingThreads = vm["readthreads"].as<unsigned int>();
  if (vm.count("parsethreads")) {
    config::parsingThreads = vm["parsethreads"].as<unsigned int>();
  }else{
    config::parsingThreads = std::max(1u, (unsigned int)(((std::thread::hardware_concurrency())-config::readingThreads)/config::readingThreads));
  }
  ...
  if (vm.count("history.persistent")) {
...
void ConfigParser::dumpConfig() {
  ...
  std::cout << "indices.bloom.approx_count:"<< config::bloom_count <<std::endl;
 ...
 // std::cout << "sim.merge_on_parse:"<< config::sim_merge_on_parse <<std::endl;
