...
void joda::network::client::JodaClient::browseReturn(unsigned long id,
  ...
  bool exec = true;
  auto vec =
      ResultRequest::result(client, prefix, id, currMin, chunkSize, true);
  while (exec) {
    getmaxyx(stdscr, h, w);
    DLOG(INFO) << "i: " << i << " currMin: " << currMin
               << " currMax: " << currMax << " chunkSize: " << chunkSize
               << " |vec|: " << vec.size();
    DLOG(INFO) << "h: " << h << " w: " << w;
    ...
    if (i < vec.size()) {
      ...
      DLOG(INFO) << "Spatial size of json string (h,w): (" << strh << ","
                 << strw << ")";
    ...
    std::string statusstr = "ID: " + std::to_string(id) + " showing " +
                            std::to_string(currMin + i + 1) + " of " +
                            std::to_string(max);
      ...
      case KEY_LEFT:
        ...
        if (i == 0) {
          if (currMin != 0) {
            ...
            vec = ResultRequest::result(client, prefix, id, currMin, chunkSize,
                                        true);
            i = vec.size() - 1;
            jh = jw = 0;
          }
...
void joda::network::client::JodaClient::getStringWindowSize(
    const std::string& str, int& y, int& x) {
  y = 1;
  x = 1;
  int currx = 0;
  for (auto&& ch : str) {
...
void joda::network::client::JodaClient::nonInteractiveQuery(
    const std::string& query, unsigned long offset, unsigned long count) {
  try {
    ...
    auto currcount = std::min(
        count,
        static_cast<unsigned long>(JODA_CLIENT_NON_INTERACTIVE_BATCH_SIZE));

    while (count > 0) {
      LOG(INFO) << "Getting result " << id << ". Retrieving" << currcount
                << " entries, starting at " << curroffset << ".";
      auto jsons = ResultRequest::result(client, prefix, id, curroffset,
                                         currcount, true);
      LOG(INFO) << "Retrieved " << jsons.size() << " results.";
      count -= currcount;
      curroffset += currcount;
      std::min(
          (size - offset) - curroffset,
          static_cast<unsigned long>(JODA_CLIENT_NON_INTERACTIVE_BATCH_SIZE));
...
joda::network::client::JodaClient::JodaClient(const std::string& addr, int port)
    : client(addr, port) {
...
void joda::network::client::JodaClient::checkError(
  ...
  if (!error.IsString()) {
    throw(JodaServerAPIErrorException(
        "'error' is not a string, invalid API response"));
  }
...
rapidjson::Document joda::network::client::JodaClient::parseResponse(
  ...
  if (doc.HasParseError()) {
    ...
    throw(JodaServerInvalidJSONException(
