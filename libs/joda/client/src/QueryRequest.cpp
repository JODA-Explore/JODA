...
std::pair<int64_t, int64_t> joda::network::client::QueryRequest::query(httplib::Client &client,
  ...
  if (!response.HasMember("success")) {
    throw (joda::network::client::JodaServerAPIErrorException("Missing 'success' attribute in response"));
  }
  auto &success_ = response["success"];
  if (!success_.IsNumber()) {
    throw (joda::network::client::JodaServerAPIErrorException("Invalid type of 'success' attribute in response"));
  }
  auto success = success_.GetInt64();

  if (!response.HasMember("size")) {
    throw (joda::network::client::JodaServerAPIErrorException("Missing 'size' attribute in response"));
  }
  auto &size_ = response["size"];
  if (!size_.IsNumber()) {
    throw (joda::network::client::JodaServerAPIErrorException("Invalid type of 'size' attribute in response"));
  }
  auto size = size_.GetInt64();

  if (size < 0) {
    if (!response.HasMember("message")) {
      throw (joda::network::client::JodaServerAPIErrorException("Missing 'message' attribute in response"));
    }
    auto &message_ = response["message"];
    if (!message_.IsString()) {
      throw (joda::network::client::JodaServerAPIErrorException("Invalid type of 'message' attribute in response"));
    }
