...
class QueryParsingTest : public ::testing::Test {
 protected:
  ...
  const std::unique_ptr<joda::query::Predicate> default_choosePred = std::make_unique<joda::query::ValToPredicate>(true);
  const std::vector<std::unique_ptr<joda::docparsing::IImportSource>> default_importSources = {};
  ...
  const std::vector<std::unique_ptr<joda::query::ISetProjector>> default_setproj = {};
  ...
  void equalImportSource(const std::unique_ptr<joda::docparsing::IImportSource> &lhs, const std::unique_ptr<joda::docparsing::IImportSource> &rhs) {
  ...
  void checkEqual(const std::shared_ptr<joda::query::Predicate> &toTest, const std::unique_ptr<joda::query::Predicate> &control) {
  ...
  void equals(std::shared_ptr<joda::query::Query> &q,
              const std::string &loadVar,
              const std::vector<std::unique_ptr<joda::docparsing::IImportSource>> &importSources,
    ...
    if (exportDestination == nullptr) {
      ...
      EXPECT_TRUE(q->getExportDestination() != nullptr);
      EXPECT_STREQ(exportDestination->toString().c_str(), q->getExportDestination()->toString().c_str());
    }
    ...
    for (int i = 0; i < q->getSetProjectors().size(); ++i) {
      EXPECT_STREQ(q->getSetProjectors()[i]->toString().c_str(), setproj[i]->toString().c_str());
    }

    //Agg
    EXPECT_EQ(agg.size(), q->getAggregators().size());
    for (int i = 0; i < q->getAggregators().size(); ++i) {
      EXPECT_STREQ(q->getAggregators()[i]->getDestPointer().c_str(), agg[i]->getDestPointer().c_str());
      EXPECT_STREQ(q->getAggregators()[i]->getName().c_str(), agg[i]->getName().c_str());
    }
...
TEST_F(QueryParsingTest, LoadFile) {
  ...
  sources.push_back(std::make_unique<joda::docparsing::JSONFileSource>("json.json", false, 1));
  equals(q,
         "A",
         sources,
         default_exportDestination,
...
TEST_F(QueryParsingTest, LoadFiles) {
  ...
  EXPECT_NE(q, nullptr);
  std::vector<std::unique_ptr<joda::docparsing::IImportSource>> sources;
  sources.push_back(std::make_unique<joda::docparsing::JSONFileDirectorySource>("tmp", false, 1));
  equals(q,
         "A",
         sources,
         default_exportDestination,
...
TEST_F(QueryParsingTest, LoadFileSample) {
  ...
  EXPECT_NE(q, nullptr);
  std::vector<std::unique_ptr<joda::docparsing::IImportSource>> sources;
  sources.push_back(std::make_unique<joda::docparsing::JSONFileSource>("json.json", false, 0.5));
  equals(q,
         "A",
         sources,
         default_exportDestination,
...
FROM FILES "tmp2" LINESEPARATED
  ...
  std::vector<std::unique_ptr<joda::docparsing::IImportSource>> sources;
  sources.push_back(std::make_unique<joda::docparsing::JSONFileSource>("json.json", false, 0.5));
  sources.push_back(std::make_unique<joda::docparsing::JSONFileDirectorySource>("tmp", false, 0.2));
  sources.push_back(std::make_unique<joda::docparsing::JSONFileSource>("one.json", true, 1));
  sources.push_back(std::make_unique<joda::docparsing::JSONFileDirectorySource>("tmp2", true, 1));
  equals(q,
         "A",
         sources,
         default_exportDestination,
...
TEST_F(QueryParsingTest, LoadFileSample2) {
  std::shared_ptr<joda::query::Query> q;
  EXPECT_NO_THROW(q = parseQuery("LOAD A FROM FILE \"json.json\" LINESEPARATED SAMPLE 0.382783464824757"););
  EXPECT_NE(q, nullptr);
  std::vector<std::unique_ptr<joda::docparsing::IImportSource>> sources;
  sources.push_back(std::make_unique<joda::docparsing::JSONFileSource>("json.json", true, 0.382783464824757));
  equals(q,
         "A",
         sources,
         default_exportDestination,
...
TEST_F(QueryParsingTest, SimpleStore) {
  ...
  std::unique_ptr<IExportDestination> e = std::make_unique<StorageExport>(store);
  equals(q,
         "A",
         default_importSources,
         e,
...
TEST_F(QueryParsingTest, StoreFILE) {
  ...
  EXPECT_NE(q, nullptr);
  std::unique_ptr<IExportDestination> e = std::make_unique<FileExport>("json.json");
  equals(q,
         "A",
         default_importSources,
         e,
...
TEST_F(QueryParsingTest, ChooseFalse) {
  std::shared_ptr<joda::query::Query> q;
  auto falsePred = IValToPred(std::make_unique<joda::query::BoolProvider>(false));
...
TEST_F(QueryParsingTest, QueryToStringTest) {
  std::shared_ptr<joda::query::Query> q;
  std::string qstring = R"(LOAD A FROM FILES "/dir" CHOOSE '' == 5.000000 AS ('/old':'') AGG ('':SUM('/old')))";
...
TEST_F(QueryParsingTest, NumParseTest) {
  std::shared_ptr<joda::query::Query> q;
  std::string qstring = R"(LOAD A CHOOSE ('' == 5.000000 || ('' == 3 || ('' == -4 || '' == -4.789000))))";
  EXPECT_NO_THROW(q = parseQuery(qstring));
  EXPECT_STREQ(qstring.c_str(), q->toString().c_str());
...
TEST_F(QueryParsingTest, MixedAggTest) {
  ...
  EXPECT_STREQ(aggs[1]->toString().c_str(), "'/count':GROUP COUNT('/old') BY EXISTS('')");
  EXPECT_STREQ(aggs[2]->toString().c_str(), "'/dist':DISTINCT('/old')");
  EXPECT_STREQ(aggs[3]->toString().c_str(), "'/collect':GROUP COLLECT('/old') AS coll BY ''");
}